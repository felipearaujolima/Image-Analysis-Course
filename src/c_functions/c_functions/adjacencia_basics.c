...
{
    Adjacencia* adj = (Adjacencia*)calloc(1, sizeof(Adjacencia)); /* criando um ponteiro com um elemento e do tamanho da estrutura adjacencia 
...
void DestroiAdjacencia(Adjacencia** adj) /* aqui precisamos de um ponteiro para um ponteiro para uma adjacencia 
...
Adjacencia* Retangular(int w, int h) /* aqui criamos a adjacencia retangular */
{
    Adjacencia* adj = NULL; /* instanciamos uma estrutura vazia */

    if (w % 2 == 0) w += 1;  /* se for par, adicionamos mais um */
    if (h % 2 == 0) h += 1; /* o mesmo aqui */

    adj = CriaAdjacencia(w * h); /* criamos uma adjacencia, mas note que o tamanho do grid vai ser esse */
